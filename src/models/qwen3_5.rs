...
use crate::models::mtp::{discover_mtp_layers, Qwen3_5MultiTokenPredictor};
...
impl Qwen3_5ForCausalLM {
    ...
    pub fn new_with_prefix(
        ...
        let (mtp_predictor, mtp_lm_head) = if mtp_num_tokens > 0 {
            ...
            let mtp_prefix = format!(
                "{}mtp",
                if mtp_prefix.is_empty() || mtp_prefix == "mtp" {
                    ""
                } else {
                    "."
                }
            ...
            if !has_mtp_module {
                crate::log_warn!(
                    "MTP enabled but no MTP module weights found at '{}'. Disabling MTP.",
                    mtp_prefix
                );
                (None, None)
            } else {
                ...
                let predictor =
                    Qwen3_5MultiTokenPredictor::new(mtp_vb.clone(), comm.clone(), config, dtype);
                ...
                match predictor {
                    Ok(predictor) => {
                        ...
                        match mtp_lm_head {
                            ...
                            Err(e) => {
                                crate::log_error!(
                                    "Failed to load MTP lm_head: {:?}. Disabling MTP.",
                                    e
                                );
                                (None, None)
                            }
    ...
    pub fn is_mtp_enabled(&self) -> bool {
        self.mtp_num_tokens > 0 && self.mtp_predictor.is_some()
    }
    ...
    /// Generate MTP draft tokens autoregressively using the MTP predictor head.
    /// `last_token`: the token just sampled from the main model.
    /// `last_hidden`: the last hidden state from the main model forward pass.
    /// `position`: the sequence position of `last_token`.
    /// `num_draft_tokens`: how many speculative tokens to propose.
    /// Returns `(draft_token_ids, draft_hidden_states)`.
    pub fn mtp_propose(
        &self,
        last_token: u32,
        last_hidden: &Tensor,
        ...
        let predictor = match &self.mtp_predictor {
            Some(p) => p,
            None => candle_core::bail!("MTP predictor not loaded"),
        };
        let lm_head = match &self.mtp_lm_head {
            Some(h) => h,
            None => candle_core::bail!("MTP lm_head not loaded"),
        };
        ...
        for step in 0..num_draft_tokens {
            ...
            let mtp_hidden = predictor.forward(
                &token_tensor,
                &pos_tensor,
                &current_hidden,
                input_metadata,
    ...
    /// Run the main model forward pass and return both logits and the
    /// last-position hidden state (needed to seed MTP drafting).
