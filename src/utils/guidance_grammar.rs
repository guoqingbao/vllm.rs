...
pub enum ToolFormat {
    ...
    Json,
    Generic,
    Gemma4,  // NEW: For Gemma-4 channel-style tool calls (call:function_name{args} format)
}
...
impl ToolCallGrammar {
    ...
    pub fn new_qwen_xml(tools: Vec<Tool>, start_token_id: u32, end_token_id: u32) -> Self {
        Self {
            tools,
            start_token_id,
            end_token_id,
    ...
    pub fn for_model_type(
        ...
        let format = if let Some(name) = parser_name {
            ...
            match model_type {
                ...
                | ModelType::Qwen3VL => ToolFormat::QwenXml,
...
impl GrammarBuilder for ToolCallGrammar {
    fn build_lark(&mut self) -> String {
        match self.format {
            ToolFormat::QwenXml => self.build_xml_lark(),
...
"#
    ...
    fn build_xml_lark(&mut self) -> String {
        let mut rules: Vec<String> = Vec::new();
...
impl<'a> GrammarRequestDispatcher<'a> {
    ...
    fn build_tool_grammar(&self) -> Option<ToolCallGrammar> {
        ...
        match self.parser_name.as_str() {
            "qwen_coder" => Some(ToolCallGrammar::new_qwen_xml(
                    tools,
                    start_token_id,
                    end_token_id,
                )),
...
/*
    ...
    fn test_for_model_type_with_override() {
        ...
        assert!(matches!(grammar.format, ToolFormat::QwenXml));
